...
class PriorityQueue<E, P> {
    ...
    public Node add(E e, P priority) {
        // Instantiate a new node with the element, its priority, and current tree size as its index
        Node node = new Node(e, priority, tree.size());
        
        // Append the newly created node to the end of the underlying tree structure
        tree.add(node);
        
        // Restore the heap property by moving the newly added node to its correct position
        // This ensures that the highest priority element is always at the root
        pullUp(tree.size() - 1);
        
        // Return the newly created and positioned node
        return node;
    }
    ...
    public boolean contains(E e) {
        // Initialize a flag to track whether the element is found in the tree
        boolean contain = false;
        
        // Iterate through all nodes in the underlying tree structure
        // This is a linear search through the entire data structure
        for (int i = 0; i < tree.size(); i++) {
            // Retrieve the node at the current index
            Node node = tree.get(i);
            
            // Check if the node exists and its value matches the searched element
            // Uses equals() for object comparison to handle different object types
            if (node != null && node.value().equals(e)) {
                // If a matching node is found, set the flag to true
                contain = true;
                
                // Optional: Could break the loop early once a match is found
                // break;
            }
        }
